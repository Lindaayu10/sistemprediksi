<?php defined('BASEPATH') OR exit('No direct script access allowed');

class Model_ujiakurasi extends CI_Model
{
    private $_table = "uji_akurasi";
    public $id_uji;
    public $nama_siswa;
    public $jenkel;
    public $pengetahuan;
    public $ketrampilan;
    public $spiritual;
    public $sosial;
    public $predikat_asli;

    //cek validasi
    public function rules()
    {
        return [

            ['field' => 'nama_siswa',
            'label' => 'nama_siswa',
            'rules' => 'required'],

            ['field' => 'jenkel',
            'label' => 'Jenkel',
            'rules' => 'required'],

            ['field' => 'pengetahuan',
            'label' => 'Pengetahuan',
            'rules' => 'required'],

            ['field' => 'ketrampilan',
            'label' => 'Ketrampilan',
            'rules' => 'required'],

            ['field' => 'spiritual',
            'label' => 'Spiritual',
            'rules' => 'required'],

            ['field' => 'predikat_asli',
            'label' => 'predikat_asli',
            'rules' => 'required'],

        ];
    }

    //fungsi cek session
    function logged_id()
    {
        return $this->session->userdata('id_admin');
    }

    //fungsi check username & password login
    function check_login($table, $field1, $field2, $field3, $field4, $field5, $field6, $field7, $field8)
    {
        $this->db->select('*');
        $this->db->from($table);
        $this->db->where($field1);
        $this->db->where($field2);
        $this->db->where($field4);
        $this->db->where($field5);
        $this->db->where($field6);
        $this->db->where($field7);
        $this->db->where($field8);
        $this->db->limit(1);
        $query = $this->db->get();
        if ($query->num_rows() == 0) {
            return FALSE;
        } else {
            return $query->result();
        }
    }

    //get all data admin
    public function getAll()
    {
        return $this->db->get($this->_table)->result();
    }

    //fungsi edit
    public function getById($id_uji)
    {
        return $this->db->get_where($this->_table, ["id_uji" => $id_uji])->row();
    }

    //ngesave inputan new ke database
    public function save()
    {
        $post = $this->input->post();
        $this->id_uji = $post["id_uji"];
        $this->nama_siswa = $post["nama_siswa"];
        $this->jenkel = $post["jenkel"];
        $this->pengetahuan = $post["pengetahuan"];
        $this->ketrampilan = $post["ketrampilan"];
        $this->spiritual = $post["spiritual"];
        $this->sosial = $post["sosial"];
        $this->predikat_asli = $post["predikat_asli"];
        
          // $password = md5($this->input->post('password'));

        $this->db->insert($this->_table, $this);
    }

    //ngeupdate inputan edit ke database
    public function update()
    {
        $post = $this->input->post();
        $this->id_uji = $post["id_uji"];
        $this->nama_siswa = $post["nama_siswa"];
        $this->jenkel = $post["jenkel"];
        $this->pengetahuan = $post["pengetahuan"];
        $this->ketrampilan = $post["ketrampilan"];
        $this->spiritual = $post["spiritual"];
        $this->sosial = $post["sosial"];
        $this->predikat_asli = $post["predikat_asli"];
        
        
        $this->db->update($this->_table, $this, array('id_uji' => $post['id_uji']));
    }
    
    //hapus data database
    public function delete($id_uji)
    {
      return $this->db->delete($this->_table, array("id_uji" => $id_uji));
    }

}
