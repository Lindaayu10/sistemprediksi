  ...
  <!-- Dropdown - User Information -->
    ...
    <!-- End of Topbar -->
        ...
        <div class="text-center justify-content-between mb-4">
         <h1 class="h3 mb-0 text-black-800">DATA TESTING</h1>
        </div>
       ...
       // $db_object = $this->db->load;
        ...
        if (isset($_POST['submit'])) {
            // if(!$input_error){
            $data = new Spreadsheet_Excel_Reader($_FILES['file_data_testing']['tmp_name']);
...
//                }
            ...
            if($result){
                ?>
                <script> location.replace("?admin=datatesting&pesan_success=Data berhasil disimpan");</script>
                <?php
            }
            else{
                ?>
                <script> location.replace("?guru=datatesting&pesan_error=Data gagal disimpan");</script>
                <?php
            }
        }

        if (isset($_POST['delete'])) {
            $sql = "TRUNCATE datatesting";
            $this->db->query($sql);
            ?>
            <script> location.replace("?guru=datatesting&pesan_success=Data latih berhasil dihapus");</script>
            <?php
        }
      ...
      // echo "<pre>"; var_dump($jumlah); echo "</pre>"; die();
        ...
        <div class="row">
            <div class="col-md-12">
                <!--UPLOAD EXCEL FORM-->
                <form method="post" enctype="multipart/form-data" action="<?= site_url('guru/Datatesting/excel') ?>">
                    <div class="form-group">
                       
                            <label>Import data from excel</label>
                            <input name="file_datatesting" type="file" class="form-control">
                        
                    </div>
                    <div class="form-group">
                        <input name="submit" type="submit" value="Upload Data" class="btn btn-success">
                    
                        <a href="<?= base_url('guru/datatesting/delete') ?>" class="btn btn-danger">
