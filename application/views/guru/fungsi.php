...
function can_access_menu($menu){
    if($_SESSION['sistemprediksi']==2 & ($menu=='prediksi')){// || $menu=='view_rule')){
        return true;
    }
    if($_SESSION['sistemprediksi']==1 & (($menu=='data_trining')
             || ($menu=='uji_akurasi') || ($menu=='hasil') || ($menu=='datasiswa') || $menu=='data_soal')){
        return true;
    }
    return false;
}

function get_id_datasiswa($db_object, $id_datasiswa){
    $sql = "SELECT id_dataadmin FROM datasiswa "
            . " WHERE id=".$id_datasiswa;
    $result = $db_object->db_query($sql);
    $row = $db_object->db_fetch_array($result);
    return $row[0];
}

function get datasiswa($db_object, $id_datasiswa){
...
function sudah_klasifikasi($db_object, $id_datasiswa){
    $sql = "SELECT COUNT(*) FROM prediksi "
            . " WHERE id_datasiswa = ".$id_datasiswa;
    $result = $db_object->db_query($sql);
    $row = $db_object->db_fetch_array($result);
    return $row[0]>0;
}

function get_hasil($db_object, $id_datasiswa){
    $sql = "SELECT * FROM hasil WHERE id_datasiswa = ".$id_datasiswa;
    $result = $db_object->db_query($sql);
    $row = $db_object->db_fetch_array($result);
    return $row;
}
//START USERS===================================================================
/**
 * combobox list datadmin dengan text didepannya (for input form)
 ...
 */
function text_with_list_dataadmin($label='', $name='', $value='', $required=false, $all=false, $text_all='-',$where='', $params='')
{
//	start_row();
...
/**
 * data list dataadmin (from db: dataadmin)
 * @param string $name
 ...
 */
function list_dataadmin($name, $selected_id='', $required=false, $all=false, $text_all='-', $where='', $params=''){
        $db_obj = new database();
        
	$table = TB_PREF."dataadmin";
