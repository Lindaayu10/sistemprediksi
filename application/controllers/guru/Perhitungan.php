...
class Perhitungan extends CI_Controller
{
    ...
    {
        if($this->model_perhitungan->logged_id())
        {   
            $data["perhitungan"] = $this->db->get('hasil')->result_array();
            $data["akurasi"] = $this->db->from('akurasi')->order_by('id_akurasi', 'DESC')->get()->result()[0] ?? [];
            $this->load->view("guru/perhitungan", $data); 
        }else{
       ...
       public function naivebayes()
       {
        ...
        $this->db->insert('akurasi',$akurasi);
				 ...
				 $this->model_perhitungan->storeModel($insertModel);
        ...
        redirect($_SERVER['HTTP_REFERER']);
			...
			public function testData()
			{
				...
				$model = $this->model_perhitungan->getModel();
       ...
       public function excel()
    {
        ...
        // ); echo "</pre>"; die();
            ...
            $this->db->insert_batch('perhitungan', $data);
    ...
    public function add()
    {
        $perhitungan = $this->model_perhitungan;
        $validation = $this->form_validation;
        $validation->set_rules($perhitungan->rules());
    ...
    {
        if (!isset($id_uji)) redirect('guru/perhitungan');
        ...
        $data["perhitungan"] = $perhitungan->getById($id_uji);
        if (!$data["perhitungan"]) show_404();
    ...
    public function delete()
    {
        if ($this->db->empty_table('hasil')) {
            $this->db->empty_table('akurasi');
            redirect(site_url('guru/perhitungan'));
        }
    }
